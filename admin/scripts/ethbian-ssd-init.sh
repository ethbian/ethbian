...
echo ""
echo "*****************************************"
echo "*        ETHBIAN SSD CONFIG v0.2        *"
echo "*****************************************"
echo ""

# ------------------------------ functions ----------------------
function new_drive_init () {
...
function mount_partition() {
  echo 'Found previously used partition(s).'
  echo 'Trying to check & mount it...'
  echo ''

  PARTITIONS=`blkid |grep 'GETH_DATADIR' |cut -f 1 -d ":"`
  if [ $LABELLED -ne 1 ]; then
    echo "             detected partitions:"
    echo ""
    for PARTITION in $PARTITIONS; do
      DESC=`sudo fdisk -l $PARTITION| head -1`
      echo $PARTITION [$DESC]
    done
    echo ""
    echo "Which one would you like to use? (full path, eg. /dev/sda1)"
    read PARTITION
    if [ ! -b $PARTITION ]; then
      echo ""
  ...
  echo "Trying to mount $PARTITION to see if datadir directory exists..."

  sudo mount $PARTITION /mnt/ssd
  if [ $? -ne 0 ]; then
    echo -e "\n   mounting $PARTITION error\n"
    exit 1
  fi

  if [ ! -d /mnt/ssd/datadir ]; then
    echo 'Directory datadir does not exist on the partition.'
    echo 'Please try again.'
  ...
  sudo umount /mnt/ssd
  echo 'The partition looks good. Trying to add it to the fstab file...'
  
  SSD_PARTUUID=`lsblk -no PARTUUID $PARTITION`
  if [ ${#SSD_PARTUUID} -eq 0 ]; then
    echo -e "\n   error getting PARTUUID\n"
    exit 1
  fi
  ...
  sudo chown -R eth:eth /mnt/ssd/datadir
  if [ $? -ne 0 ]; then
    echo -e "\n   changing ownership of /mnt/ssd/datadir error\n"
    exit 1
  fi
...
LABELLED=`blkid |grep -c 'GETH_DATADIR'`
if [ $LABELLED -ne 0 ]; then
  mount_partition
else
  new_drive_init
fi
